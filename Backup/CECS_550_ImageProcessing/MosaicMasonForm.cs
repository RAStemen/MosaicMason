...
using System.Xml;
using System.IO;
using CECS_550_ImageProcessing.Filters;
            ...
            int width, int height, string cellColorArray, bool compareAsGrayscale)
        {
            ...
            for (int j = 0; j < height; j++)
            {
                for (int i = 0; i < width; i++)
                {
                    ...
                    masterColor = XmlHelper.RGBStringToColor(masterColorArray.Substring(index, 6));
                    index = 6 * (i + (j * width));
                    cellColor = XmlHelper.RGBStringToColor(cellColorArray.Substring(index, 6));
                    totalDifference += FilterHelper.TotalColorDiff(masterColor, cellColor, compareAsGrayscale);
                }
            }
